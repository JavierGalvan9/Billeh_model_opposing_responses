...
class SparseLayer(tf.keras.layers.Layer):
    def __init__(self, indices, weights, dense_shape, bkg_weights, lr_scale=1., dtype=tf.float32, **kwargs):
        ...
        self._max_batch = int(2**40 / weights.shape[0]) #int(2**31 / weights.shape[0])
        self._dtype = dtype
    ...
    def call(self, inp):
        ...
        noise_input = tf.reshape(
            noise_input, (shp[0], shp[1], -1))
        
        # print(input_current.shape, rest_of_brain.shape, noise_input.shape)
        
        return input_current, noise_input
...
class BillehColumn(tf.keras.layers.Layer):
    def __init__(self, network, input_population, bkg_weights, neurons_selection,
        ...
        self.state_size = (
            ...
            n_receptors * self._n_neurons,                   # psc rise_rec
            n_receptors * self._n_neurons,                   # psc_rec
            n_receptors * self._n_neurons,                   # psc rise ext
            n_receptors * self._n_neurons,                   # psc ext
            n_receptors * self._n_neurons,                   # psc rise bkg
            n_receptors * self._n_neurons,                   # psc bkg
        )
        
        # self.heatmap_indices = neurons_selection['Tf index']
        # self.keller_type = neurons_selection['Keller type']
    ...
    def zero_state(self, batch_size, dtype=tf.float32):
        ...
        # psc0 = tf.zeros(
        #     (batch_size, self._n_neurons * self._n_receptors), dtype)
        psc_rise_rec0 = tf.zeros(
            (batch_size, self._n_neurons * self._n_receptors), dtype)        
        psc_rec0 = tf.zeros(
            (batch_size, self._n_neurons * self._n_receptors), dtype)
        psc_rise_ext0 = tf.zeros(
            (batch_size, self._n_neurons * self._n_receptors), dtype)
        psc_ext0 = tf.zeros(
            (batch_size, self._n_neurons * self._n_receptors), dtype)
        psc_rise_bkg0 = tf.zeros(
            (batch_size, self._n_neurons * self._n_receptors), dtype)
        psc_bkg0 = tf.zeros(
            (batch_size, self._n_neurons * self._n_receptors), dtype)
        ...
        return z0_buf, v0, r0, asc_10, asc_20, psc_rise_rec0, psc_rec0, psc_rise_ext0, psc_ext0, psc_rise_bkg0, psc_bkg0
    ...
    def call(self, inputs, state, constants=None):
        ...
        if constants is not None:
            if self._spike_gradient:
                # print('model_output_currents')
                # print(self._n_neurons, self._n_receptors, self._n_neurons*self._n_receptors)
                # print(inputs.shape)
                external_current = inputs[:,
                                          :self._n_neurons * self._n_receptors]
                bkg_inputs = inputs[:,
                                          self._n_neurons * self._n_receptors:2*self._n_neurons * self._n_receptors]
                state_input = inputs[:, 2*self._n_neurons * self._n_receptors:]
            else:
                external_current = inputs[:,
                                          :self._n_neurons * self._n_receptors]
                bkg_inputs = inputs[:,
                ...
                state_input = tf.reshape(
                    state_input, (batch_size, self._n_neurons, 4))
        # external_current = inputs
        z_buf, v, r, asc_1, asc_2, psc_rise_rec, psc_rec, psc_rise_ext, psc_ext, psc_rise_bkg, psc_bkg = state
        ...
        psc_rise_rec = tf.reshape(
            psc_rise_rec, (batch_size, self._n_neurons, self._n_receptors))
        psc_rec = tf.reshape(psc_rec, (batch_size, self._n_neurons, self._n_receptors))
        psc_rise_ext = tf.reshape(
            psc_rise_ext, (batch_size, self._n_neurons, self._n_receptors))
        psc_ext = tf.reshape(psc_ext, (batch_size, self._n_neurons, self._n_receptors))
        
        psc_rise_bkg = tf.reshape(
            psc_rise_bkg, (batch_size, self._n_neurons, self._n_receptors))
        psc_bkg = tf.reshape(psc_bkg, (batch_size, self._n_neurons, self._n_receptors))
        ...
        ### MINE ###
        # i_rec2 = sparse_w_rec.__mul__(tf.cast(rec_z_buf, tf.float32))
        # i_rec2= tf.sparse.transpose(i_rec2) 
        
        # print(type(sparse_w_rec), sparse_w_rec.shape) # sparse_tensor, (20000, 20000)
        # print(type(tf.cast(rec_z_buf, tf.float32)), tf.cast(rec_z_buf, tf.float32).shape) #eager tensor, (1, 20000)
        # print(type(i_rec), i_rec.shape) #eager tensor, (1, 20000)
        # print(type(i_rec2), i_rec2.shape) # sparse_tensor, (20000, 20000)
        # try:
        #     a = tf.sparse.reduce_sum(i_rec2, axis=0, output_is_sparse=False)
        #     print(np.sum(a - i_rec))
        # except:
        #     pass
          
        # rec_inputs2 = tf.cast(i_rec2, self._compute_dtype)
        # rec_inputs2 = tf.sparse.reshape(
        #     rec_inputs2, (batch_size, self._n_neurons, self._n_receptors, self._n_neurons, self.max_delay))
        # try:
        #     c = tf.cast(i_rec, self._compute_dtype)
        #     a = tf.reshape(c, (batch_size, self._n_neurons, self._n_receptors))
        #     b = tf.sparse.reduce_sum(rec_inputs2, [-2, -1])
        #     print(np.sum(a-b))
        # except:
        #     pass
        
        # rec_inputs2 = rec_inputs2 * self._lr_scale
        # no tiene sentido sumarle el state input a las dos...
        # if constants is not None and not self._spike_gradient:
        #     external_current2 = external_current + state_input * self._lr_scale
        #     rec_inputs2 = rec_inputs2 + state_input * self._lr_scale
        
        # external_current2 = tf.reshape(
        #     external_current, (batch_size, self._n_neurons, self._n_receptors))
        # external_current2 = external_current2 * self._lr_scale
        # We do not include the state_input since its value is 0 and its shape is (1, 5000)
        
        # rec_inputs2 = tf.sparse.reduce_sum(rec_inputs2, [-3, -1])
        # rec_inputs2 = tf.cast(rec_inputs2, self._compute_dtype)
        # external_current2 = tf.reduce_sum(external_current2, -1)
        # external_current2 = tf.cast(external_current2, self._compute_dtype)        
                
        # new_indices = tf.constant([[0, 1920], [0, 1960]])
        # ext_sel = tf.gather(external_current2, indices=self.heatmap_indices, axis=1)[-1]
        # rec_sel = tf.gather(rec_inputs2, indices=self.heatmap_indices, axis=1)[-1]
        # print(rec_inputs2.shape)  #(1, 5000, 5000)
        # print(rec_sel.shape) #(30, 5000)
        # print(ext_sel.shape) #(30,)
        # try:
        #     print(self._lr_scale, np.sum(state_input)) # (1.0, 0.0)
        # except:
        #     pass
        
        # e23_dict = {}
        # for idx, tf_idx in enumerate(self.heatmap_indices):
        #     e23_dict[str(tf_idx)] = {'recurrent':tf.reshape(rec_sel[idx], (batch_size, self._n_neurons)), 
        #                              'external':tf.reshape(ext_sel[idx], (batch_size, 1))
        #                              }
        # e23_dict = {"id1920": {'recurrent': tf.reshape(rec_sel[0], (batch_size, self._n_neurons)),
        #                       'external': tf.reshape(ext_sel[0], (batch_size, 1))},
        #            "id1960": {'recurrent': tf.reshape(rec_sel[1], (batch_size, self._n_neurons)),
        #                       'external': tf.reshape(ext_sel[1], (batch_size, 1))}
        #            }
        
        # print(external_current2)
        # print(i_rec2)        
        # new_psc_rise2 = psc_rise2 * self.syn_decay + i_rec2 * self.psc_initial
        # new_psc2 = psc2 * self.syn_decay + self._dt * self.syn_decay * psc_rise2
        # new_psc_rise2_ext = psc_rise2_ext * self.syn_decay + external_current2 * self.psc_initial
        # new_psc2_ext = psc2_ext * self.syn_decay + self._dt * self.syn_decay * psc_rise2_ext
            
        # if constants is not None and self._spike_gradient:
        #     input_current_rec = tf.reduce_sum(psc2, 2) + state_input
        # else:
        #     input_current_rec = tf.reduce_sum(psc2, 2)
        ############

        rec_inputs = tf.cast(i_rec, self._compute_dtype)
        external_current = tf.cast(external_current, self._compute_dtype)
        bkg_inputs = tf.cast(bkg_inputs, self._compute_dtype)
        ...
        ext_inputs = tf.reshape(
            external_current, (batch_size, self._n_neurons, self._n_receptors))
        bkg_inputs = tf.reshape(
            bkg_inputs, (batch_size, self._n_neurons, self._n_receptors))
        rec_inputs = rec_inputs * self._lr_scale
        ext_inputs = ext_inputs * self._lr_scale
        bkg_inputs = bkg_inputs * self._lr_scale
        ###
        
        # if constants is not None and not self._spike_gradient:
        #     rec_inputs = rec_inputs + state_input * self._lr_scale
            
        # print(psc_rise.shape) #(1, 5000, 4)
        # print(rec_inputs.shape) #(1, 5000, 4)
        # print(psc.shape) #(1, 5000, 4)
        # print(state_input.shape) #(1, 5000)
        ...
        new_psc_rise_rec = psc_rise_rec * self.syn_decay + rec_inputs * self.psc_initial
        new_psc_rec = psc_rec * self.syn_decay + self._dt * self.syn_decay * psc_rise_rec
        new_psc_rise_ext = psc_rise_ext * self.syn_decay + ext_inputs * self.psc_initial
        new_psc_ext = psc_ext * self.syn_decay + self._dt * self.syn_decay * psc_rise_ext
        new_psc_rise_bkg = psc_rise_bkg * self.syn_decay + bkg_inputs * self.psc_initial
        new_psc_bkg = psc_bkg * self.syn_decay + self._dt * self.syn_decay * psc_rise_bkg

        input_current_rec = tf.reduce_sum(psc_rec, -1)
        input_current_ext = tf.reduce_sum(psc_ext, -1)
        input_current_bkg = tf.reduce_sum(psc_bkg, -1)
        input_current = input_current_rec + input_current_ext + input_current_bkg
        
        new_input_current_rec = tf.reduce_sum(new_psc_rec, -1)
        new_input_current_ext = tf.reduce_sum(new_psc_ext, -1)
        new_input_current_bkg = tf.reduce_sum(new_psc_bkg, -1)
        new_input_current = new_input_current_rec + new_input_current_ext + new_input_current_bkg
        ...
        # if constants is not None and self._spike_gradient:
        #     input_current = tf.reduce_sum(psc_rec + psc_ext, -1) + state_input
        # else:
        #     input_current_rec = tf.reduce_sum(psc_rec, -1)
        #     input_current_ext = tf.reduce_sum(psc_ext, -1)
        #     input_current = input_current_rec + input_current_ext     
        ...
        # new_psc = tf.reshape(
        #     new_psc, (batch_size, self._n_neurons * self._n_receptors))
        # new_psc_rise = tf.reshape(
        #     new_psc_rise, (batch_size, self._n_neurons * self._n_receptors))       
        new_psc_rec = tf.reshape(
            new_psc_rec, (batch_size, self._n_neurons * self._n_receptors))
        new_psc_rise_rec = tf.reshape(
            new_psc_rise_rec, (batch_size, self._n_neurons * self._n_receptors))
        new_psc_ext = tf.reshape(
            new_psc_ext, (batch_size, self._n_neurons * self._n_receptors))
        new_psc_rise_ext = tf.reshape(
            new_psc_rise_ext, (batch_size, self._n_neurons * self._n_receptors))
        new_psc_bkg = tf.reshape(
            new_psc_bkg, (batch_size, self._n_neurons * self._n_receptors))
        new_psc_rise_bkg = tf.reshape(
            new_psc_rise_bkg, (batch_size, self._n_neurons * self._n_receptors))
        ...
        # z_sel = tf.gather(new_z, indices=self.heatmap_indices, axis=1)[-1] 
        # v_sel = tf.gather(new_v * self.voltage_scale + self.voltage_offset, indices=self.heatmap_indices, axis=1)[-1] 
        # input_current_sel = tf.gather(input_current + new_asc_1 + new_asc_2, indices=self.heatmap_indices, axis=1)[-1] 
                
        # for idx, tf_idx in enumerate(self.heatmap_indices):
        #     e23_dict[str(tf_idx)]['z'] = tf.reshape(z_sel[idx], (batch_size, self._n_neurons))
        #     e23_dict[str(tf_idx)]['v'] = tf.reshape(v_sel[idx], (batch_size, self._n_neurons))
        #     e23_dict[str(tf_idx)]['input_current'] = tf.reshape(input_current_sel[idx], (batch_size, self._n_neurons))
            

        
        # outputs = (new_z, new_v * self.voltage_scale + self.voltage_offset)
        # print((input_current + new_asc_1 + new_asc_2).shape)  #(1, 5000)
        # print(external_current2.shape) #(1, 5000)
        # outputs = (new_z, new_v * self.voltage_scale + self.voltage_offset, input_current + new_asc_1 + new_asc_2, e23_dict)
        
        # print(new_z.shape) # (1, 5000)
        # print(self.voltage_scale.shape) #(5000,)
        # print(new_input_current_rec.shape)
        # print(new_input_current.shape)
        # print((new_input_current_rec*self.voltage_scale).shape)
        outputs = (new_z, new_v * self.voltage_scale + self.voltage_offset, 
                   (new_input_current + new_asc_1 + new_asc_2)*self.voltage_scale, 
                   new_input_current_rec*self.voltage_scale, new_input_current_ext*self.voltage_scale)

        new_state = (new_z_buf, new_v, new_r, new_asc_1, new_asc_2, 
                     new_psc_rise_rec, new_psc_rec, new_psc_rise_ext, new_psc_ext,
                     new_psc_rise_bkg, new_psc_bkg)
...
def create_model(network, input_population, bkg_weights, selection_data_path, seq_len=100, n_input=10, n_output=2,
    ...
    state_input = tf.cast(tf.identity(state_input_holder), dtype) # tf.cas() changes the dtype

    if batch_size is None:
        batch_size = tf.shape(x)[0] # fi batch_size is None just update after each timestep (1 ms in our case)
    else:
        batch_size = batch_size
        
    # e23_df = pd.DataFrame(file_management.load_lzma(selection_data_path))
    # neurons_selection = e23_df.loc[e23_df['heatmap_neurons'] == True]
    neurons_selection = None
    cell = BillehColumn(network, input_population, bkg_weights, neurons_selection,
    ...
    lgn_inputs, bkg_inputs = SparseLayer(
        cell.input_indices, cell.input_weight_values, cell.input_dense_shape,
        cell.bkg_weights, lr_scale=lr_scale, dtype=dtype, name='input_layer')(x)

    # rnn_inputs = tf.cast(rnn_inputs, dtype)
    # rnn_inputs = tf.cast(lgn_inputs + bkg_inputs, dtype)
    # rnn_inputs = tf.cast(lgn_inputs, dtype)
    
    # print(lgn_inputs.shape)
    # print(bkg_inputs.shape)
    # print(state_input.shape)
    full_inputs = tf.concat((tf.cast(lgn_inputs, dtype), tf.cast(bkg_inputs, dtype), state_input), -1)
    # full_inputs = [tf.cast(lgn_inputs, dtype), tf.cast(bkg_inputs, dtype), state_input]
    ...
    # print(hidden[2].shape)      #input_current + new_asc_1 + new_asc_2 (None, 2500, 5000)
    # print(hidden[3].shape)      #rec_sel (1, 2500, 5000)
    # print(hidden[4].shape)      #ext_sel (1, 2500)
