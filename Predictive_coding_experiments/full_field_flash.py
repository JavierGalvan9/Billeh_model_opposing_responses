...
from plotting_utils import InputActivityFigureWithoutStimulus, RasterPlot, LaminarPlot, LGN_sample_plot, PopulationActivity
import load_sparse
import models
import models_output_currents
import toolkit
# import callbacks
# import data_sets
...
def main(_):
    ...
    path = os.path.join(flags.save_dir, f'full_field_flashes_rec_{flags.neurons}_black_flash_{flags.black_flash}')
    ...
    ### MODEL INPUT ###
    lgn_firing_rates_filename = f'black_to_white_{not(flags.black_flash)}&init_screen_dur_1.0&flash_dur_1.0&end_screen_dur_1.0&min_value_-1&max_value_1&contrast_0.8&dt_0.001&height_120&width_240.lzma'
    with open(os.path.join(flags.data_dir, 'input', 'Full_field_flash', lgn_firing_rates_filename), 'rb') as f:
    ...
    if flags.output_currents and os.path.exists(full_selection_data_path):
        model = models_output_currents.create_model(
            ...
            use_state_input=True, return_state=True)
    ...
    graph = InputActivityFigureWithoutStimulus(network, flags.data_dir, image_path, filename=raster_filename, 
...
if __name__ == '__main__':
    ...
    absl.app.flags.DEFINE_boolean('black_flash', False, '')
